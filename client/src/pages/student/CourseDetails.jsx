...
import YouTube from 'react-youtube'; // Assuming you are using react-youtube for video playback
const CourseDetails = () => {
  const { id } = useParams(); // Used to get URL parameters from the current URL.
  ...
  const { allCourses ,calculateRating,currency,
    calculateChapterTime,calculateCourseDuration,calculateNoOfLectures} = useContext(AppContext); // Get the context values from the AppContext to get courses info.

  const fetchCourseData = () => {
    // Match the course using '_id', which seems to be the key provided by your data structure
    const findCourse = allCourses.find(course => course._id === id);
    console.log("Found Course:", findCourse); // Debugging: Ensure correct course is found
    setCourseData(findCourse);
  };


  useEffect(() => {
    if (allCourses.length > 0) { // Ensure there are courses before attempting to find one
      console.log("All Courses:", allCourses); // Verify that courses are being loaded
      fetchCourseData();
    }
  }, [allCourses, id]); // Include 'id' in dependencies in case it changes
    ...
    <>
      <div className="flex md:flex-row flex-col-reverse gap-10 relative items-start justify-between md:px-36 px-8 md:pt-20 pt-10 text-left">
        ...
        <h1 className="md:text-course-deatails-heading-large text-course-deatails-heading-small font-semibold text-gray-800">
          ...
          <p className="pt-4 md:text-base text-sm" dangerouslySetInnerHTML={{ __html: courseData.courseDescription.slice(0, 233) }}></p>
